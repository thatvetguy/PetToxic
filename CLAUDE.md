...
- **No single-newline markdown lists in `description` or `toxicityInfo` fields.** `GlossaryStyledText` splits on `\n\n` for paragraph breaks, and `AttributedString(markdown:)` swallows single `\n`, so bullet items separated by a single `\n` run together with no visible separation. **Prohibited:** `- item\n- item`. **Acceptable:** bullets separated by blank lines (`- item\n\n- item` — each renders as its own paragraph; several audited entries use this), or better, `\n\n`-separated paragraphs with bold headers (`**EARS:**`, `**NOSE:**`, etc.) / inline prose. `Scripts/validate_content.py` enforces exactly this line (single-`\n` bullets = error). Applies to `DatabaseService.swift` and `DiseasesConditionsService.swift`. The `symptoms` and `preventionTips` arrays are unaffected — they render as individual list rows.
